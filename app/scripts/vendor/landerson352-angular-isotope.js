...
angular.module("iso.controllers", ["iso.config", "iso.services"]).controller("angularIsotopeController", [
  "iso.config", "$scope", "$timeout", "optionsStore", function(config, $scope, $timeout, optionsStore) {
    ...
    $scope.init = function(isoInit) {
      ...
      //watch the options object and push changes
      if(isoInit.options) $scope.$watchCollection(isoInit.options, function(newValue, oldValue) {
        $scope.updateOptions(newValue);
      });
    ...
    $scope.reLayout = function() {
      if (postInitialized) {
        return isotopeContainer.isotope('reLayout');
      }
    };
    $scope.updateOptions = function(option) {
      var isNewSort = ensureSortMethodExists(option);
      if (isotopeContainer) {
        if(isNewSort && postInitialized) {
    ...
    function ensureSortMethodExists(options) {
      if(!options.getSortData) options.getSortData = {};
      if(!options.getSortData[options.sortBy]) {
       addSortMethod(options.getSortData, options.sortBy);
       return true;
      }
      return false;
    }
    function addSortMethod(sortData, key) {
      sortData[key] = function ( $elem ) {
        var repeater = $elem.attr('ng-repeat');
        if(repeater) {
          var itemPropertyName = repeater.split(' in ')[0];
          return $elem.scope()[itemPropertyName][key];
        }
        return null;
      };
    }
